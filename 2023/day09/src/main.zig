...
pub fn day09_2(input: []const u8) !i32 {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    ...
    while (lines.next()) |report_line| {
        ...
        var next_num: i32 = 0;
        var reverse_rows = std.mem.reverseIterator(prediction_table.items);
        while (reverse_rows.next()) |row| {
            next_num = row.items[0] - next_num;
        }
        result += next_num;
    }
...
test "example 2" {
    ...
    const output = try day09_2(input);
    try std.testing.expectEqual(@as(i32, 2), output);
}

test "input 2" {
    const input = @embedFile("./input09");
    const output = try day09_2(input);
    try std.testing.expectEqual(@as(i32, 1062), output);
}
